...
const LoginPage = ({ theme = 'dark', onClose, onLoginSuccess = () => {} }) => {
  ...
  return (
    <motion.div
      ...
      className="relative backdrop-blur-xl rounded-2xl p-10 shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto border"
    ...
    >
      ...
      <h2 className="text-4xl font-bold mb-2 text-center" style={{ color: currentColors.primaryText }}>
        {isLoginMode ? 'Welcome back' : 'Create Account'}
      </h2>
      <p className="text-center mb-10" style={{ color: currentColors.primaryText, opacity: 0.8 }}>
        {isLoginMode ? 'Login to your account' : 'Join ECHO today!'}
      </p>

      <div className="flex justify-center mb-8 rounded-full p-1" style={{ backgroundColor: currentColors.secondaryBg }}>
        <button
          onClick={() => setIsLoginMode(true)}
          className="flex-1 py-2 px-4 rounded-full text-sm font-medium transition-all duration-300"
          style={isLoginMode
            ? { backgroundColor: currentColors.accentGold, color: currentColors.secondaryText, boxShadow: '0 1px 2px rgba(0,0,0,0.15)' }
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div>
          <label htmlFor="email" className="block text-sm font-medium mb-2" style={{ color: currentColors.primaryText, opacity: 0.9 }}>Email</label>
          <input
            type="email"
            id="email"
            name="email"
        ...
        <div>
          <label htmlFor="password" className="block text-sm font-medium mb-2" style={{ color: currentColors.primaryText, opacity: 0.9 }}>Password</label>
        ...
        {!isLoginMode && (
          <div>
            <label htmlFor="confirm-password" className="block text-sm font-medium mb-2" style={{ color: currentColors.primaryText, opacity: 0.9 }}>Confirm Password</label>
            <div className="relative">
              <input
        ...
        <button
          type="submit"
          className="w-full p-3 mt-8 rounded-lg font-semibold shadow-md transition-colors duration-300 transform hover:scale-105"
      ...
      <div className="flex items-center justify-center my-8" style={{ color: currentColors.primaryText, opacity: 0.7 }}>
        <span className="h-px w-1/4 bg-current"></span><span className="mx-4">or</span><span className="h-px w-1/4 bg-current"></span>
      </div>

      <div className="space-y-4">
        <button
          className="w-full p-3 rounded-lg flex items-center justify-center font-semibold transition-colors duration-300 border"
          style={{ borderColor: currentColors.darkGold, backgroundColor: currentColors.primaryBg, color: currentColors.primaryText }}
        >
